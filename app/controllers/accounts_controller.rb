class AccountsController < ApplicationController
  ...
  before_action :load_account, except: [:settings, :update_settings,:index,:following_acc]

  def index
    @accounts = Account.all_except(current_account).limit(8).reverse
    render json: @accounts.as_json,status: 200
  end
  ...
  def following_acc
    @follow_accs = current_account.to_follower
    render json: @follow_accs.as_json,status: 200
  end
